...
def _factor(rate_percent: Decimal, du: int) -> Decimal:
    base = Decimal(1) + rate_percent / Decimal(100)
    exponent = Decimal(du) / Decimal(252)
    return (base.ln() * exponent).exp()


def _annualize(factor: Decimal, du: int) -> Decimal:
    if du <= 0:
        raise ValueError("DU_target must be positive")
    return ((factor.ln() * (Decimal(252) / Decimal(du))).exp() - 1) * Decimal(100)
...
def flat_forward_interpolate(
    ...
    with localcontext() as ctx:
        ...
        factor_previous = _factor(rate_previous, du_previous)
        factor_next = _factor(rate_next, du_next)
        factor = factor_previous * (
            (factor_next / factor_previous).ln() * weight
        ).exp()
        return _annualize(factor, du_target)
...
def flat_forward_extrapolate(
    ...
    with localcontext() as ctx:
        ...
        factor_previous = _factor(rate_previous, du_previous)
        factor_last = _factor(rate_last, du_last)
        factor = factor_previous * (
            (factor_last / factor_previous).ln() * weight
        ).exp()
        return _annualize(factor, du_target)
